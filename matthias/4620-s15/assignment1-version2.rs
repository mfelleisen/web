...
fn main() {
    ...
    let (above,below) : (usize,usize) = above_below(mean,raw_data);
    println!("{}\n{}\n{}\n",mean,above,below);
}
...
fn average(raw_data : & Vec<f64>) -> f64 {
    let l : usize = raw_data.len();
    if 0 == l {
       return RETURN_FOR_EMPTY_SEQUENCE;
    }
    else {
        // we have to clone here to get .into_iter() to work 
        let new_data   = raw_data.clone();
        return new_data.into_iter().map(|x| x).sum() / l as f64;
    }
}
