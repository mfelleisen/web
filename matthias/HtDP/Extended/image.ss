#| TODO
   ...
   Mode = { 'solid, 'outline }
    ...
    circle ;; Number Color Mode -> Image 

   Robby will change the error-value->string-handler
   so that snip doesn't show up anymore, but image.
|#
(module image mzscheme
  ...
  (provide ;; forall(World):
   ...
   frame       ;; Number Number Color -> Image
  ...
  (provide on-void-key-event)
  (define (on-void-key-event f)  
  ...
  (define (rectangle width height color)
    ...
    (check-color 'rectangle color "third")
    (beg:filled-rect width height color))
  
  (define (frame width height color)
    (check-pos 'rectangle width "first")
    (check-pos 'rectangle height "second")
    (check-color 'rectangle color "third")
    (beg:outline-rect width height color))
  
  (define (circle radius color)
    (check-pos 'circle radius "first")
    (check-color 'circle color "second")
    (beg:outline-circle radius radius color))
  
  (define (disk radius color)
    (check-pos 'disk radius "first")
    (check-color 'disk color "second")
  ...
  (define (run-movie movie)
    ...
    (let run-movie ([movie movie])
      (cond [(beg:empty? movie) #t]
            [(beg:cons? movie)
             (update (beg:first movie) produce #t)
             (sleep/yield .05)
             (run-movie (beg:rest movie))])))
  ...
  (define (start w h)
    ...
    (when the-frame (error 'start "start already called once"))
    (set! the-frame
          (new (class frame%
                 (super-new)
