;; Cellular Automata: Beginners who know accumulators (for state-next)
...
(define (state-next s) 
  (cons (cell-next 'white (first s) (second s))
        (state-next-aux (rest (rest s)) (first s) (second s))))

;; State Cell Cell -> State 
;; accumulator style: HtDP VI
;; combine left and right neigbors with this cell to create new cell
(define (state-next-aux s left this)
  (cond 
    [(empty? s) (list (cell-next left this 'white))]
    [else  (cons (cell-next left this (first s)) 
                 (state-next-aux (rest s) this (first s)))]))

(equal? 
 (state-next (list 'black 'black))
 (list 'black 'white))

;; State -> Image
;; turn a state into an image; fill to WIDTH elements first!
