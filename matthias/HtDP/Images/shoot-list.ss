...
(define-struct platform (posn shots))
;; Platform = (create-platform Posn (Listof Shot))
...
;; Number -> Platform
(define (create-platform n) (make-platform (make-posn n 10) empty))

;; Platform UFO -> Boolean
(define (hit-shot? p ufo)
  (ormap (lambda (p) 
           (and (<= (posn-x ufo) (posn-x p) (+ (posn-x ufo) UFO-WIDTH))
                (<= (posn-y p) (posn-y ufo) (+ (posn-y p) SHOT-LENGTH))))
         (platform-shots p)))
...
;; ... ... ... Posn Posn -> true
(define (fff-platform solid-disk solid-rect line p qs)
  (and
   (solid-rect (make-posn (posn-x p) (- HEIGHT 3)) (posn-y p) 3 'blue)
   (andmap (lambda (q) (solid-rect q SHOT-WIDTH SHOT-LENGTH 'red)) qs)))
...
(define (draw-platform p)
  (fff-platform draw-solid-disk draw-solid-rect draw-solid-line 
                (platform-posn p) (platform-shots p)))
...
(define (move-platform p d)
  (make-platform (posn+ (platform-posn p) (make-posn d 0)) (platform-shots p)))

;; Platform -> Platform 
(define (move-shot p)
  (make-platform 
   (platform-posn p)
   (map (lambda (s) (posn+ s (make-posn 0 -10))) (platform-shots p))))

;; Platform -> Platform 
(define (fire-platform p)
  (make-platform (platform-posn p) 
                 (cons (mid-platform (platform-posn p)) (platform-shots p))))
